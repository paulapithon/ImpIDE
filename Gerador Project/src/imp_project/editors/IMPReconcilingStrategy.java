...
public class IMPReconcilingStrategy implements IReconcilingStrategy, IReconcilingStrategyExtension {
		...
		while (cNextPos < fRangeEnd) {
			while (cNextPos < fRangeEnd) {
				...
				case '{':
					...
					String tagString = fDocument.get(startOffset,
							Math.min(cNextPos - startOffset, fRangeEnd - startOffset)); // this is to see where we are
																						// in the debugger
	 ...
	 * after &lt;, when it returns, it points to the character after &gt; or after
	 * the range. About syntax errors: this method is not a validator, it is useful.
	 * Side effect: writes number of found newLines to {@link #cNewLines}.
	 *
		...
		try {
			...
			if (';' == ch) {
				while (cNextPos < fRangeEnd) {
					...
					/*
					 * if (';' == ch) { ch = fDocument.getChar(cNextPos++); while ((cNextPos <
					 * fRangeEnd) && (';' != ch)) { ch = fDocument.getChar(cNextPos++); }
					 */
					if ('\'' == ch) {
						ch = fDocument.getChar(cNextPos++);
